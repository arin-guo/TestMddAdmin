 ...
 * 记录行为日志
 */
function writeSchoolLog($type = 0){
	//过滤登录类
	if(strtolower(request()->controller()) == 'login' && $type == 0){
		return true;
	}
	$MemberLog = Db::name('MemberLog');
	$uid = session('user_school_id');$flag = 0;
	$data['user_id'] = $uid;
	$data['username'] = session('user_school_tel');
	$data['realname'] = session('user_school_realname');
		...
		foreach ($actionList as $val){
			...
			if(request()->controller()  == $moduleName[0]){
				switch (request()->action()){
					...
					default :$data['title'] = $val['title'];$data['url'] = request()->controller().'/'.request()->action();$data['behavior'] = "其他操作";$flag = 1;;
				}
			}
		}
	}
